...
const targetUPIField = `<h2 className="text-xl font-bold text-primary mb-1">Payment Details</h2>
...
const bankDetailsForm = `<h2 className="text-xl font-bold text-primary mb-1">Bank Details</h2>
...
if (content.includes('Payment Details')) {
  content = content.replace(targetUPIField, bankDetailsForm);
  fs.writeFileSync('src/pages/ProfessionalOnboarding.jsx', content);
  console.log('Restored Bank Details successfully');
} else {
  console.log('Could not find Payment Details block');
}
